...
module.exports = function(options) {
    return function(req, res, next) {
        if (req.url.indexOf('/__api__/proxy/') === 0 || req.url.indexOf('/proxy/') === 0) {
            ...
            // Re-Build the location header for redirect responses, so browsers can follow them
            ...
            // Note that the Host header will be replaced as expected by the proxy itself 
            // (because of the changeOrigin flag)
