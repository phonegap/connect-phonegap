...
function getVDirOverride(mapping, device)
{
	...
	var vplatform = options.vdir[device.platform];
	return (typeof vplatform === "string") 
		? vplatform 
		: vplatform[device.version.replace('.', '_')];
}
 
function getVDirPrefix(options, req)
{
	return (options.vdir && getVDirOverride(options.vdir, req.session.device)) 
			|| path.join(__dirname,
...
module.exports = function(options) {
    return function(req, res, next) {        
        var prefix = getVDirPrefix(options, req);
