    ...
    return function(req, res, next) {
        if (req.url.indexOf('/__api__/appzip') === 0) {
            ...
            if (req.method === 'HEAD') {
                ...
                var injectScript = function(options) {
                    ...
                    // append homepage/refresh (3/4 finger taps)
                    if (typeof options.homepage !== 'boolean') {
                        options.homepage = true;
                    }

                    if(options.homepage) {
                        scripts += fs.readFileSync(homepageScript);
                    }
                    ...
                    if(options.refresh) {
                        scripts += fs.readFileSync(refreshScript);
                    }
