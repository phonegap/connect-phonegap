...
module.exports = function(options) {
    return function(req, res, next) {
        if (req.url.indexOf('/__api__/appzip') === 0 && req.method === 'GET') {
            ...
            var theWalker  = walk.walk(path.join(process.cwd(), 'www'), { followLinks: true })

            theWalker.on('file', function(root, fileStat, nextFile){
                var filename = path.join(root, fileStat.name);
                var output = filename.split(process.cwd())[1];
                // ensure that symlinks have their real file-path
