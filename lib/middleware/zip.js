            ...
            if (req.method === 'HEAD') {
                ...
                var injectScript = function() {
                    var deployScript = path.join(resPath, 'deploy.js'),
                        ...
                        socketServerScript = path.join(resPath, "socket-server.js");
                    ...
                    var scripts = fs.readFileSync(deployScript) +
                                  ...
                                  fs.readFileSync(socketServerScript);
                                  fs.readFileSync(pushScript) +
                                  fs.readFileSync(refreshScript);

                    // adding connected devices variable 
                    if(req.session && req.session.device) {
                        scripts += fs.readFileSync(path.join(resPath, "socket-server.js"));
                    }
