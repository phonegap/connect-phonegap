...
var connect = require('connect'),
    ...
    cordovaAPI = require('../util/cordova-API-access');
...
module.exports = function(options) {
    ...
    if (options.watch) {
        options.watch.on('all', function(event, filepath) {
            getCordova(options.emitter, function(error, cordova) {
                console.log('GOT1')
                if (!error) {
                    console.log('prep1')
    ...
    if (fs.existsSync(path.join(process.cwd(), 'platforms/browser'))) {
        getCordova(options.emitter, function(error, cordova){
            console.log('GOT')
            if (!error) {
                cordova.prepare([], function(e, data) {console.log('PREP')});
            }
        });
    }
...
module.exports.addBrowserPlatform = function(options) {
    if (!fs.existsSync(path.join(process.cwd(), 'platforms/browser'))) {
        ...
        getCordova(options.emitter, function(error, cordova) {
            if (!error) {
                cordova.platform('add', 'browser', function(e, data) {
                    cordova.prepare([], function(e, data) {});
                });
            }
        });
...
function generateBrowserJson(browserPlatform) {
    ...
    // this fixes an issue when creating a new project from desktop app
    if (!files.length)
        return;
...
/**
 * Get cordova from project node_modules
 * @param  {EventEmitter}   emitter  
 * @param  {Function} callback(error, cordova)
 * @return {None} 
 */
function getCordova(emitter, callback){
    console.log('getCordova')
    return cordovaAPI.getCordova(emitter)
    .then(function(cordova){
        callback(undefined, cordova);
    })
    .fail(function(error){
        options.emitter.emit('error', 'could not find cordova dependency in project');
        callback(error);
    });
};