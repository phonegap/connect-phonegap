...
module.exports = function(options) {
    ...
    return function(req, res, next) {
        if (req.url.indexOf('/__api__/autoreload') === 0) {
            ...
            // clear out the files that need to be updated for a particular session
            // /__api__/update will set the files to update to [] once it 200s
            if(req.session.lastUpdated > lastModified) 
            {
                if(req.session.filesToUpdate.length === 0) {
                    activeSessions.set(req.sessionID, req.session.filesToUpdate);
                }
            }
