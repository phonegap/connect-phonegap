...
module.exports = function(options) {
    ...
    return function(req, res, next) {
        if (req.url.indexOf('/__api__/autoreload') === 0) {
            ...
            if (browserToggle && isBrowser(req)) {
                req.session.lastUpdated = 0;
                browserToggle = false;
            }
...
// Helper function to detect whether the XHR is coming from the browser platform
function isBrowser(req) {
    return (/Macintosh|Windows NT/).test(req.headers['user-agent']);
}