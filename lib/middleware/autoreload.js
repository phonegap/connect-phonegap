...
module.exports = function(options) {
    ...
    return function(req, res, next) {
        if (req.url.indexOf('/__api__/autoreload') === 0) {
            if (req.method === 'GET') {
                // when lastUpdated is undefined, set it the current date
                // because the user has only been served the zip file and
                // has not made a call to /__api__/autoreload
                if (!req.session.lastUpdated) {
                    req.session.lastUpdated = lastModified;
                }
            }
