...
module.exports = {
    getScripts: function(options) {
        ...
        var scripts = [
            ...
            { enabled: (!options.isBrowser), src: path.join(__dirname, '../../res/middleware/deploy.js') },             // device
            { enabled: (options.homepage || true), src: path.join(__dirname, '../../res/middleware/homepage.js') },           // either
            { enabled: (options.isBrowser && (options.proxy || true)), src: path.join(__dirname, '../../res/middleware/proxy.js') },              // browser
            { enabled: (options.push || true), src: path.join(__dirname, '../../res/middleware/push.js') },               // either
            { enabled: (options.refresh || true), src: path.join(__dirname, '../../res/middleware/refresh.js') }             // either
        ];
