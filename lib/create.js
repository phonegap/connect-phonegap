...
module.exports.downloadTemplate = function(options, callback, url) {
    var destinationPath = module.exports.templatePath(options);
	if (url === undefined) {
        var url = 'https://github.com/phonegap/phonegap-app-hello-world/archive/master.tar.gz';
    }
...
module.exports.createProject = function(options, callback) {
    ...
    if (fs.existsSync(configXML.wwwPath)) {
        ...
        configXML = {
            projectPath: path.join(options.path, 'config.xml'),
            sourcePath: path.join(module.exports.templatePath(options), 'config.xml')
        };
        fs.renameSync(configXML.sourcePath, configXML.projectPath);
    }
